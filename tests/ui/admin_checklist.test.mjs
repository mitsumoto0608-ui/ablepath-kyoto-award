...
test("[source_conformance] facility and plaza row counts equal the receipt record counts times their attribute counts", () => {
  ...
  // Permission-excluded shelters are not plazas. D5 has exactly these two categories.
  const expectedPlazaCategories = ["emergency_open_space", "temporary_stay_facility"];
  for (const cityId of ["kyoto_kiyomizu", "kyoto_arashiyama"]) {
    ...
    assert.deepEqual(items.filter((item) => item.object_type === "plaza").map((item) => item.object_id).sort(), expectedPlazaCategories.map((category) => `${cityId}:${category}`));
  }
...
test("[fatal] exposure is never converted into damage, debris or closure state", () => {
  ...
  for (const cityId of CITY_IDS) {
    ...
    for (const item of checklist.items) {
      ...
      assert.equal(item.exposure_flags.status, "PUBLIC_PAYLOAD_EXCLUDED");
      assert.equal(item.exposure_flags.records, null, "excluded evidence is not zero exposure");
      assert.equal(item.exposure_flags.a31b_overlap_counts, null);
      assert.ok(item.exposure_flags.reason.length > 0);
      assert.doesNotThrow(() => validateExposureFlags(item.exposure_flags));
      assert.throws(() => validateExposureFlags({ ...item.exposure_flags, records: 0 }), /excluded/);
