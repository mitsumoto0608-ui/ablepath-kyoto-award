...
def test_kyoto_facility_source_fields_preserve_listed_values_and_unknown_runtime_state() -> None:
    """[source_conformance] Approved public subset: 77 historical - 49 unbound = 28 toilet rows, with original attributes and UNKNOWN runtime state."""
    ...
    assert len(payload["records"]) == 28
    assert payload["historical_record_count"] == 77 and payload["excluded_record_count"] == 49
    by_source = {}
    ...
    assert set(by_source) == {"kyoto_public_toilets_00307"}
