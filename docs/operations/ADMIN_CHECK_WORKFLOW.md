...
## Current public projection — 2026-09-13

Authority: `reports/PUBLIC_SAFE_SNAPSHOT_SCOPE.json`. Historical source totals are not current public counts. Kyoto public facility rows are toilets only (18 Kiyomizu–Gion, 10 Arashiyama); 49 shelter records are excluded. Fujisawa public facility rows remain 0. No substitute rows are invented. D5 plaza rows remain only `emergency_open_space` and `temporary_stay_facility`; permission-excluded shelter categories do not become plazas.

Individual PLATEAU/exposure rows are excluded pending provider binding. Every edge checklist carries `PUBLIC_PAYLOAD_EXCLUDED`, `records=null`, `a31b_overlap_counts=null` and a reason. JSON, CSV and printed UI preserve this absence, not zero exposure. The schema's narrow conditional branch and the loader reject excluded counts changed to zero. Current public row counts, original historical counts and reasoned-null evidence are separate concepts.

Public staged inputs are regenerated by `scripts/build_candidate_analysis.py`, then `npm run build:data`. Historical raw rebuild entry points refuse to overwrite this public snapshot; raw originals remain external. Git payloads and the entire CI upload trees (`reports/`, `viewer/dist/`, `viewer/test-results/`) are scanned, including untracked generated files, original workbooks and large text. Old audits/counts are historical snapshots and do not override this public projection. HUMAN_GATE 1–5 and historical public reachability remain unchanged.
