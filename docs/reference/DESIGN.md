...
- 各モジュールに**先にテストを書く**。残存幅フィクスチャは**両ケース必須**（v2.1——旧版の「両側h=7m→0.73m」は片側倒壊の限定が落ちており、そのまま実装すると誤った手計算が正解として固定されるところだった。レビュー指摘）：**ケースA**＝幅4.0m・**片側のみ**倒壊→W=max(4.0−3.27,0)=**0.73m**（車いす0.90m要はFAIL・一般可）／**ケースB**＝**両側**倒壊→W=max(4.0−3.27−3.27,0)=**0m**（全profile不可）。
...
- **mutationテスト**：最低限——片側瓦礫項の削除／m↔cm／≥↔>／UNKNOWN→OPEN／上り下り反転／mean+σ→mean−σ／(2/3)→23／容量超過割当——の各変異でテストが必ず落ちることを確認して記録（変異リストは実際にA0検証で見つかった誤り：PSのunion/majority混同・Weldのvalidation/labeling混同・(2/3)→23化け・Iskandar指標定義・停止回数の行混成、から採った）。
