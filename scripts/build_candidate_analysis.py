...
def _official_evidence(root: Path, city_id: str, edges: list[dict]) -> tuple[dict, list[dict]]:
    ...
    if city_id == "fujisawa_enoshima":
        ...
        if any(row["source_id"] != "kyoto_public_toilets_00307" for row in records):
            raise ValueError("unbound Kyoto facility rows cannot enter the public static analysis")
        ...
        for category, value in category_status["categories"].items():
            public_count = len([row for row in records if row["category"] == category]) if value["map_connected"] else 0
            categories[category] = {**value, "record_count": public_count, "public_record_count": public_count, "historical_record_count": value["historical_counts_by_aoi"][parity_group], "count_scope": parity_group}
        ...
        historical_count = sum(value["historical_record_count"] for value in categories.values())
        facility = {"status": "PARTIAL_PUBLIC_TOILET_ONLY_SOURCE_COORDINATES_AND_ORIGINAL_ATTRIBUTES", "geometry_status": "SOURCE_PROVIDED_LONGITUDE_LATITUDE", "marker_policy": "SOURCE_COORDINATES_ONLY_NO_GEOCODING", "record_count": len(records), "historical_record_count": historical_count, "excluded_record_count": historical_count - len(records), "records": records, "source_catalog": facility_source_catalog, "source_fields_sha256": _hash(source_fields_path), "categories": categories, "display_layer": {"data_path": f"./data/official/facility_points_{parity_group}.geojson", "artifact_sha256": _hash(point_path), "copied_sha256": _hash(point_path), "feature_count": len(_load_json(point_path)["features"])}, "reason": f"Current public rows: {len(records)} toilets; original selection: {historical_count}; excluded pending provider permission: {historical_count - len(records)} shelter/emergency rows. Source coordinates and listed toilet attributes are retained without geocoding. Opening, entrance, accessibility, safety and disaster usability remain UNKNOWN. Missing public rows do not mean absent facilities."}
    else:
    ...
    official = {
        ...
        "facility": facility,
        "plateau": {"status": "NOT_CONNECTED", "aoi_count": len(subareas), "aoi_scope": subareas, "inventory": plateau_inventory, "inventory_connected": bool(city_truth), "inventory_scope": "HISTORICAL_AGGREGATE_METADATA_ONLY_INDIVIDUAL_PAYLOAD_EXCLUDED", "fallback": plateau["fallback"], "m7_evidence_ready_count": plateau["m7_evidence_ready_count"], "m7_computed_count": plateau["m7_computed_count"], "reason": "Historical package/AOI aggregate metadata is retained; individual building payload is excluded pending provider redistribution binding. This does not mean no buildings or zero exposure. Real 3D and M7 remain NOT_CONNECTED; field confirmation is deferred. Existing source-traceable candidate 2D is the deterministic fallback."},
