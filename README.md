# ablepath-kyoto-kit — 清水・祇園 観光×地震・火災・大雨 検証ツール

**現在地＝シナリオ計算エンジンv0.2**：安全契約・データ検査・KPI4区分・論文定数管理・再現性を備えた計算エンジン。**同梱データはすべてダミー**（清水・祇園の模式回廊。区域指定・広場指定・容量・距離は要実データ差し替え）であり、**まだ行政PoCではない**——PoC化の条件はPATCH 03（残存幅）＋清水実データ＋現地実測＋公式広場・運用条件＋証拠付き4状態＋Cesium画面。

中心説明：**京都・清水祇園の観光地で、誰が、なぜ通れないかを証拠付きedgeで評価し、平時のアクセシブル観光（晴・雨）と、地震・地震火災・大雨それぞれの既存観光客避難計画を同じ歩行グラフで検証する行政向けツール。**
...
python -m pytest tests/ -q               # 55テスト
python -m src.runner data/               # 120ラン → results/all_runs.json, summary.md
python tools/generate_readme_metrics.py  # 下の代表ラン表を実測値で更新
```
...
- 地震scenarioの閉塞導出はAWARD期に残存幅モデル（`AI_TASKS/03`：Moya 2020式×Yu&Gardoni 2022の型）へ接続する——**scenarioを作る道具であり、被害を予言する道具ではない**（横屋2024：益城町を対象にした当該検証では最良設定で道路延長ベース最大78.8%の一致。不一致が残るため、京都で個別道路の被害を断定しない）。
...
1. `AI_TASKS/01`：実証回廊の確定と現地調査（京都在住の反復力が最大の武器）
2. `AI_TASKS/02`：ビューア移植（藤沢版viewer/index.htmlが雛形。モード＝平時観光（晴/雨）／地震／地震火災（フェーズ）／大雨）
3. AWARD期：二段階（広場→一時滞在施設）、4profile化、幅・段差・横断勾配・路面・旋回・入口（障壁カタログの3層データモデル）、turn属性wayfinding、observation台帳
