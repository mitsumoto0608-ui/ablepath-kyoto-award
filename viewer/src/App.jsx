...
function OfficialEvidencePanel({ evidence }) {
  ...
  return (
    <section className="evidence-tables official-evidence" aria-labelledby="official-evidence-title">
      ...
      {evidence.facility.marker_policy === "SOURCE_COORDINATES_ONLY_NO_GEOCODING" && <div className="table-scroll" tabIndex="0" aria-label="京都の公式座標施設一覧"><table><caption>京都公式施設 {evidence.facility.record_count}件（原本提供経緯度のみ、silent geocodingなし）</caption><thead><tr><th>ID</th><th>category</th><th>name</th><th>address</th><th>source/version</th><th>coordinate method</th><th>入口/運用/accessibility</th></tr></thead><tbody>{evidence.facility.records.map((record) => <tr key={record.facility_record_id}><th scope="row"><code>{record.facility_record_id}</code></th><td>{record.category}</td><td>{record.name}</td><td>{record.address}</td><td>{record.source_id} / {record.source_version_or_valid_as_of}</td><td>{record.coordinate_method}</td><td>{record.entrance_status} / {record.operation_status} / {record.accessibility_status}</td></tr>)}</tbody></table></div>}
            ...
            {state.view === "2d" ? (
              realMode ? (
                <MapLibreMap
                  ...
                  officialLayers={{ hazard: cityAnalysis?.official_evidence?.hazard?.display_layer ?? null, facility: cityAnalysis?.official_evidence?.facility?.display_layer ?? null }}
