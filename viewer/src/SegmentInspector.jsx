...
export function SegmentInspector({ feature, evidence, conditions, path, view, onSelectEdge, onView3d, onAdmin, onClose }) {
  ...
  return <aside className="segment-inspector" aria-labelledby="segment-title">
    ...
    {!p ? <p>候補区間を地図または一覧から選んでください。</p> : <>
      ...
      <div className="segment-actions"><button type="button" onClick={onView3d}>{view === "3d" ? "この区間へ移動" : "同じ区間を3Dで確認"}</button><a href="#review-export" onClick={onClose}>この候補経路を書き出す</a></div>
      <section><h3>原典から確認できること</h3>
        ...
        <details><summary>DEMセル標高 · {conditions.terrainProduct}（原典値）</summary>
        ...
        </details><details><summary>ハザード原典 · {conditions.scenario}</summary><p className="small-note">資料条件の切替です。経路の再探索・安全判定は行いません。</p>
        ...
        })}{!selected.hazards.length && <p>この資料条件の証拠はありません。NO_DATA ≠ 影響なし。</p>}</div></details>
      </section>
      <section className="unknown-box"><h3>まだ分からないこと</h3><p>幅・段差・横断勾配・入口・解錠・現在の運用は UNKNOWN。建物表示やハザードの重なりから補いません。</p><p>M7: NOT_COMPUTED / result null</p>{selected.readiness && <details><summary>M7の不足入力</summary><p>{selected.readiness.missing_fields.join(", ")}</p></details>}</section>
