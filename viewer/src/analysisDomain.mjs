...
const EXPECTED_CONTENT_FINGERPRINTS = {
  kyoto_kiyomizu: { hazard_catalog: "16c9f626d7c72589", facility_catalog: "836dfce99eed520f", facility_records: "135edb69fd555311" },
  kyoto_arashiyama: { hazard_catalog: "0668c41634ac0568", facility_catalog: "836dfce99eed520f", facility_records: "cea3061e9d13e1da" },
  fujisawa_enoshima: { hazard_catalog: "f881cf94aecc56e7", facility_catalog: "92cbbf02e9cb3cba", facility_records: "09612b07b5ecb5a5", scenarios: "dfdaf623e2147dd7" },
};
const EXPECTED_ANALYSIS_SHA256 = {
  fujisawa_enoshima: "5905db6ad44eac3344eddaf3677ef3e7877c2240576b695f1179588c6adc93a7",
  kyoto_arashiyama: "4e79f9e69dc12e9f5fe7bd81641ec980d122546756268cb46573784d7ae78451",
  kyoto_kiyomizu: "f1c21e5ec1c4b4d1d639ec2bc6751417fca1e9725e9f727dc3611fc49c854654",
};
...
export function validateDeliveryAnalysis(analysis, expectedCityId) {
  ...
  if (expectedCityId === "fujisawa_enoshima") {
    ...
    assert(memberBound.length === 10 && memberBound.every(([_sourceId, source]) => typeof source.source_member_id === "string" && source.source_member_id.length > 0 && ["member_path_sha256", "shp_sha256", "shx_sha256", "dbf_sha256", "txt_sha256"].every((key) => SHA256.test(source.source_member_receipt?.[key]))), "Fujisawa hazard member identity is invalid");
  }
  ...
  assert(terrain.products.every((product) => product.sample_count === terrain.samples.filter((sample) => sample.product === product.product).length), "terrain product sample count is stale");
  ...
  if (expectedCityId === "fujisawa_enoshima") {
    ...
    assert(Array.isArray(hazard.scenarios) && contentFingerprint(hazard.scenarios) === EXPECTED_CONTENT_FINGERPRINTS.fujisawa_enoshima.scenarios && JSON.stringify(hazard.scenarios.map((row) => row.dataset_id).sort()) === JSON.stringify(EXPECTED_FUJISAWA_SCENARIO_IDS) && hazard.scenarios.filter((row) => row.connected === true).length === 10 && hazard.scenarios.filter((row) => row.connected === false).length === 8 && hazard.scenarios.every((row) => row.aoi_scope === "enoshima_katase" && typeof row.reason === "string" && row.reason.length > 0 && (row.connected === false ? row.status === "NOT_CONNECTED" && row.validation_result === "CRS_REVIEW_REQUIRED" && row.crs.startsWith("CRS_CONTRADICTION:") : row.status === "SOURCE_SIDE_EDGE_OVERLAP_CONNECTED" && row.validation_result === "SOURCE_SHA_CRS_DEFINITION_AND_AOI_SELECTION_BOUND" && row.crs === "EPSG:4612" && row.license_review === "CC-BY")), "Fujisawa earthquake/liquefaction scenario contract is stale");
  }
...
export function checklistToCsv(checklist) {
  const columns = ["row_kind", "city_id", "path_key", "path_status", "checklist_status", "candidate_distance", "candidate_distance_unit", "path_reason", "filters", "sort_by", "safe_route_claim", "accessibility_claim", "admin_validated", "edge_id", "scenario_id", "relation", "coverage_status", "overlap_length_m", "metric_crs", "source_id", "source_revision", "source_sha256", "source_url", "license_status", "source_published_at", "source_valid_as_of", "source_acquired_at", "source_temporal_status_reason", "coverage_selection_sha256", "source_feature_ids", "source_classes", "reason", "limitations", "interpretation", "official_closure", "damage_state", "debris_present", "terrain_status", "terrain_sampled", "terrain_reason", "terrain_samples", "facility_status", "owner_candidate_types", "facility_record_id", "facility_name", "facility_address", "facility_source_attributes", "facility_source_attribute_entries", "current_operation_status", "entrance_status", "unlock_status", "accessibility_status", "step_free_status", "disaster_availability_status", "unknowns"];
  const explicitNullColumns = new Set(["official_closure", "damage_state", "debris_present"]);
  return [columns.join(","), ...flatRows(checklist).map((row) => columns.map((column) => csvCell(row.row_kind === "candidate_edge" && explicitNullColumns.has(column) && row[column] === null ? "null" : row[column])).join(","))].join("\n") + "\n";
}
