...
export function validateDeliveryAnalysis(analysis, expectedCityId) {
  ...
  for (const [key, checklist] of Object.entries(analysis.review_checklists)) {
    ...
    assert(checklist.rows.every((row) => Array.isArray(row.owner_candidate_types) && row.owner_candidate_types.length > 0 && Array.isArray(row.hazard_refs) && JSON.stringify([...row.hazard_refs].sort()) === JSON.stringify([...(exposureRefsByEdge.get(row.edge_id) ?? [])].sort()) && Array.isArray(row.unknowns) && new Set(row.unknowns).size === CHECKLIST_UNKNOWNS.length && JSON.stringify([...row.unknowns].sort()) === JSON.stringify([...CHECKLIST_UNKNOWNS].sort())), "review checklist contains incomplete or orphan evidence references");
...
function flatRows(checklist, includeFacilities = true) {
  ...
  for (const row of checklist.rows) {
    if (!row.hazards.length) result.push({ ...selection, row_kind: "candidate_edge", ...row, scenario_id: "NO_CONNECTED_HAZARD_ROW", relation: "NO_DATA", overlap_length_m: null, source_id: null, source_revision: null, source_sha256: null, source_classes: [], official_closure: null, damage_state: null, debris_present: null });
    for (const hazard of row.hazards) result.push({ ...selection, row_kind: "candidate_edge", ...row, ...hazard });
  }
