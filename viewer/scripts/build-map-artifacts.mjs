...
function exposureFlagsFor(edgeId, exposureByEdge, receipt) {
  if (receipt.public_exclusion_reason) return {
    status: "PUBLIC_PAYLOAD_EXCLUDED", reason: receipt.public_exclusion_reason,
    records: null, a31b_overlap_counts: null,
    landslide: "PUBLIC_PAYLOAD_EXCLUDED", earthquake: "PUBLIC_PAYLOAD_EXCLUDED",
    liquefaction: "NOT_CONNECTED", tsunami: "NOT_CONNECTED", note: EXPOSURE_NOTE,
  };
  ...
  return {
    status: "SOURCE_RECORDS", reason: null,
...
function buildCityChecklist(root, cityId, sources, includeInternalUseOnly) {
  ...
  for (const edge of r1.edges ?? []) {
    ...
    for (const attribute of EDGE_ATTRIBUTES) {
      ...
      items.push({
        ...
        ...priority,
        exposure_flags: exposureFlagsFor(edge.edge_id, exposureByEdge, r2),
  ...
  if (cityId.startsWith("kyoto_")) {
    ...
    for (const [category, entry] of Object.entries(r4Categories.categories ?? {}).sort(([left], [right]) => compareCodePoints(left, right))) {
      if (!["emergency_open_space", "temporary_stay_facility"].includes(category) || entry.map_connected !== false) continue;
